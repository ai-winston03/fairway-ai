...
import { syncForeupMemberHold, syncForeupReportingRange } from "@/lib/foreup-reporting-sync";
...
export async function POST(request: NextRequest) {
  ...
  const body = await request.json().catch(() => ({})) as { start?: string; end?: string; includeMembers?: boolean };
  ...
  try {
    const reporting = await syncForeupReportingRange(courseId, teeSheetId, start, end);
    const members = body.includeMembers ? await syncForeupMemberHold(courseId, teeSheetId) : undefined;
    return NextResponse.json({ summary: { ...reporting, members } });
  } catch (error) {
